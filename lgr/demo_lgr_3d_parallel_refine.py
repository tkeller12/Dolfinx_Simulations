...
#    Vg = fem.functionspace(mesh, ("DG", 0, (gdim,))) # use DG, 0 to compute gradient, which would be integral over element
#    Vg = fem.functionspace(mesh, ("DG", 0, (gdim,))) # use DG, 0 to compute gradient, which would be integral over element
    Vg = fem.functionspace(mesh, ("DG", 0,(gdim,))) # use DG, 0 to compute gradient, which would be integral over element
    Grad = fem.Function(Vg)
    grad_form = ufl.grad(eth[0]) + ufl.grad(eth[1]) + ufl.grad(eth[2])
#    grad_form = ufl.sqrt(ufl.dot(ufl.grad(Et_dg), ufl.grad(Et_dg)))
#    grad_form = ufl.sqrt(ufl.dot(ufl.grad(eth), ufl.grad(eth)))
    print('-'*50)
#    print(eth)
#    print(type(eth))
#    print(eth.x)
#    print(eth.x.petsc_vec)
    print(eth.x.array)
    print('-'*50)

#    grad_form = ufl.dot(ufl.grad(eth)*ufl.dx, ufl.grad(eth)*ufl.dx)
#    grad_form = ufl.dot(ufl.grad(Et_dg), ufl.grad(Et_dg))
#    grad_form = ufl.grad(Et_dg)*ufl.dx
#    grad_form = ufl.dot(ufl.grad(Et_dg), ufl.grad(Et_dg))
#    grad_form = ufl.dot(ufl.grad(Et_dg), ufl.grad(Et_dg))*ufl.dx
    grad_expr = fem.Expression(grad_form, Vg.element.interpolation_points(), comm = mesh.comm)
    Grad.interpolate(grad_expr)

    order = np.argsort(Grad.x.array)
...
#    edge_index = compute_incident_entities(mesh.topology, cell_index.astype(np.int32), 3, 1)
    edge_index = compute_incident_entities(mesh.topology, cell_index.astype(np.int32), 2, 3)
    print(edge_index)
