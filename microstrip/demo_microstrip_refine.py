...
mesh, cell, facet_tags = gmshio.read_from_msh('mesh/'+filename, comm, 0, gdim=3)
mpi_print('Done.')
...
#        refine_array = G.x.array
        refine_array = residual.x.array

        # Find cells to refine
        threshold = np.percentile(refine_array, 100-percent_refinement)
        cell_index = np.arange(len(refine_array))[refine_array > threshold]
...
#        if i == 0:
        if i < nev:
            ...
            with io.VTXWriter(mesh.comm, "sols/sols_lgr_E_%04i/eig_%04i.bp"%(run_ix,i), Et_dg) as f:
                f.write(0.0)

            with io.VTXWriter(mesh.comm, "sols/sols_lgr_B_%04i/eig_%04i.bp"%(run_ix,i), B) as f:
                f.write(0.0)

#            with io.VTXWriter(mesh.comm, "sols/sols_lgr_G_%04i/eig_%04i.bp"%(run_ix,i), G) as f:
#                f.write(0.0)

#            with io.VTXWriter(mesh.comm, "sols/sols_lgr_G_refined_locations_%04i/eig_%04i.bp"%(run_ix,i), G_refined_locations) as f:
#                f.write(0.0)

#            with io.VTXWriter(mesh.comm, "sols/sols_lgr_residual_%04i/eig_%04i.bp"%(run_ix,i), residual) as f:
#                f.write(0.0)
