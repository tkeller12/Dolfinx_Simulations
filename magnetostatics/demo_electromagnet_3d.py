...
msh, cell_tags, facet_tags = gmshio.read_from_msh(filename, comm, 0, gdim=3)
...
bc_facets_x0 = locate_entities(msh, msh.topology.dim-1, marker)
...
for facet in bc_facets:
    if facet not in bc_facets_x0:
        bc_facets_list.append(facet)

bc_facets = np.array(bc_facets_list, dtype = np.int32)
