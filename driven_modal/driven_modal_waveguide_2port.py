...
port_1_facets = dolfinx.mesh.locate_entities_boundary(mesh,dim=(tdim - 1), marker=is_port_1)
port_2_facets = dolfinx.mesh.locate_entities_boundary(mesh,dim=(tdim - 1), marker=is_port_2)
...
pec_facets = np_remove(exterior_facets, port_1_facets)
#pec_facets = np_remove(pec_facets, port_2_facets)
...
port_1_marker = dolfinx.mesh.meshtags(mesh, tdim - 1, port_1_facets, np.full(len(port_1_facets), 1, dtype=np.int32))
ds_port_1 = ufl.Measure("ds", domain=mesh, subdomain_data=port_1_marker)

port_2_marker = dolfinx.mesh.meshtags(mesh, tdim - 1, port_2_facets, np.full(len(port_2_facets), 1, dtype=np.int32))
ds_port_2 = ufl.Measure("ds", domain=mesh, subdomain_data=port_2_marker)
...
L_port_1 = 0.5 *  Y * ufl.inner(u,v) * ds_port_1(1) # impedance boundary at waveguide port

#L_port_2 = 0.5 *  Y * ufl.inner(u,v) * ds_port_2(1) # impedance boundary at waveguide port

L_inc = -2.0 * gamma * ufl.inner(TE10,v) * ds_port_1(1) # incident wave

weak_form = A + L_port_1 + L_inc# + L_port_2
...
V_ref_local = fem.assemble_scalar(fem.form(ufl.inner(E,TE10) * ds_port_1(1)))
V_inc_local = fem.assemble_scalar(fem.form(ufl.inner(TE10,TE10) * ds_port_1(1)))
